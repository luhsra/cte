// SOURCE: https://stackoverflow.com/a/65222964/4099367
...
int cte_vsprintf(char * buf, const char * fmt, va_list va)
{
    char c;
    const char *save = buf;
...
int cte_sprintf(char * buf, const char * fmt, ...)
{
    va_list va;
    va_start(va,fmt);
    int ret = cte_vsprintf(buf, fmt, va);
    va_end(va);
    return ret;
}

int cte_fdprintf(int fd, const char * fmt, ...) {
    va_list va;
    va_start(va, fmt);
    char b[512];
